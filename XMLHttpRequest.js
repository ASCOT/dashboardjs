...
exports.XMLHttpRequest = function() {
  ...
  // Http states
    ...
    var service = encodeURIComponent(settings.url);  
		
		var options = {
      'host': url.parse(settings.url).host,
      'port': port,
      'path': url.parse(settings.url).pathname + url.parse(settings.url).search,
      'method': settings.method
    };
        		
		// Use the correct request method
    request = http.request(options, function(requestResponse) {
      
      response = requestResponse;
      setState(self.HEADERS_RECEIVED);
      self.status = requestResponse.statusCode;

      //console.log('STATUS: ' + requestResponse.statusCode);
      //console.log('HEADERS: ' + JSON.stringify(requestResponse.headers));
